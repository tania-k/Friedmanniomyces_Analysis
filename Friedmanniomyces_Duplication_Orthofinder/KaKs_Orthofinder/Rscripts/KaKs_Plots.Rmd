...
# script written by Tania-Kurbessoian and Jason-Stajich
...
pdf("plots/Ks_histogram_overlap_Paralogs.pdf",width=10, height=6)
#png("plots/Ks_histogram_overlap.png")
All=read_tsv("data/all.tsv")
All_C=All%>% filter(dS <=1)

hist(Fe$dS, breaks=100, xlim=c(0,1), ylim=c(0,270), col=rgb(1,0,0,0.5), xlab="Friedmanniomyces distribution", ylab="Count", main="Paralog Distribution of dS Count in Friedmanniomyces endolithicus vs Friedmanniomyces simplex")

hist(Fs$dS, breaks=100, xlim=c(0,1), ylim=c(0,400), col=rgb(0,0,1,0.5), add=T)

legend("topright", legend=c("Friedmanniomyces endolithicus","Friedmanniomyces simplex"), col=c(rgb(1,0,0,0.5), rgb(0,0,1,0.5)), pt.cex=2, pch=15 )

#ggsave(HistogramKs,OverlapHist,width=15)
#ggsave("plots2/Ks_histogram_overlap.png", OverlapHist, width=15, height=5)
dev.off()
```
...
ggsave("plots/Ks_histogram2.png",gridplot, width=10, height=4)
...
#KsPlotBarplot2 =  "plots/Ks_boxplot2.pdf"
p<- ggplot(comboData,aes(x=species,y=dS,color=species)) + geom_boxplot(outlier.shape = NA) + geom_jitter(size = 0.6,width=0.2,alpha=0.7,shape=16) + 
        labs(title="Boxplot of Ks Frequency in Duplicated Gene Pairs Across Friedmanniomyces simplex,\n Friedmanniomyces endolithicus and Hortaea werneckii", x="Species", y="Ks Frequency") + scale_color_brewer(palette='Dark2') + theme_bw() + stat_summary(fun=mean, colour="darkred", geom="point", shape=18, size=3,show.legend = FALSE) + stat_summary(fun=mean, colour="black", geom="text", show.legend = FALSE, vjust=-0.7, aes( label=round(..y.., digits=3)))
ggsave(KsPlotBarplot,p,width=12) 
ggsave("plots/Ks_boxplot.png",p, width=8, height=4)


#ggsave(KsPlotBarplot2,p,width=12) 
#ggsave("plots/Ks_boxplot2.png",p, width=8, height=4)
...
d<- ggplot(comboData2,aes(x=species,y=dN,color=species)) + geom_boxplot(outlier.shape = NA) + geom_jitter(size = 0.6,width=0.2,alpha=0.7,shape=16) + labs(title="Boxplot of Ka Frequency in Duplicated Gene Pairs Across Friedmanniomyces simplex, Friedmanniomyces endolithicus and Hortaea werneckii", x="Species", y="Ka Frequency") + theme_bw() + scale_color_brewer(palette='Dark2') + theme_bw() + stat_summary(fun=mean, colour="darkred", geom="point", shape=18, size=3,show.legend = FALSE) + stat_summary(fun=mean, colour="black", geom="text", show.legend = FALSE, vjust=-0.7, aes( label=round(..y.., digits=3)))
ggsave(KaPlotBarplot,d,width=12) 
ggsave(KaPlotBarplot,d,width=12)
ggsave("plots/Ka_boxplot.png",d, width=8, height=4)
...
p<- ggplot(comboData,aes(x=species,y=dS,color=species)) + geom_boxplot(outlier.shape = NA) + geom_jitter(size = 0.6,width=0.2,alpha=0.7,shape=16) + 
        labs(title=sprintf("Boxplot of Ks Frequency in Duplicated Gene Pairs with Alignment < %d",MAXLEN), x="Species", y="Ks") + scale_color_brewer(palette='Dark2') + theme_bw()
