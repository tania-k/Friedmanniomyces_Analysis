...
p1 <- ggplot(data=removed_na, aes(x=COG, y=Counts, fill=COG)) + geom_bar(stat="identity", width=1) + geom_text(aes(label = Counts), position=position_dodge(0.9),vjust=-0.2) + guides(fill = guide_legend(ncol = 1)) + xlab("COG Class")  + ggtitle("COG Results of Friedmanniomyces \nendolithicus Genes") + scale_x_discrete(labels=xticks) + 
...
p2 <- ggplot(data=simp.removed_na, aes(x=COG, y=Counts, fill=COG)) + geom_bar(stat="identity", width=1) + geom_text(aes(label = Counts), position=position_dodge(0.9),vjust=-0.2) + guides(fill = guide_legend(ncol = 1)) + xlab("COG Class")  + ggtitle("COG Results of Friedmanniomyces \nsimplex Genes") + scale_x_discrete(labels=xticks) + 
scale_fill_manual(values = getPalette(colourCount)) +  theme_bw()
...
p3 <- ggplot(data=Wer_removed_na, aes(x=COG, y=Counts, fill=COG)) + geom_bar(stat="identity", width=1) + geom_text(aes(label = Counts), position=position_dodge(0.9),vjust=-0.2) + guides(fill = guide_legend(ncol = 1)) + xlab("COG Class")  + ggtitle("COG Results of Hortaea \nwerneckii Genes") + scale_x_discrete(labels=xticks) + 
scale_fill_manual(values = getPalette(colourCount)) +  theme_bw()
p3
dev.off()
```
...
q1 <- ggplot(data=removed_na, aes(x=COG, y=Counts, fill=COG))+ geom_bar(stat="identity", width = 0.9, position=position_dodge()) + geom_text(aes(label = Counts), position=position_dodge(0.9),vjust=-0.2) + guides(fill = guide_legend(ncol = 1)) + xlab("COG Class") + ggtitle("COG Results of Friedmanniomyces endolithicus \nDuplicated Genes") + scale_x_discrete(labels=xticks) + 
scale_fill_manual(values = getPalette(colourCount)) +  theme_bw()
q1
dev.off()
```
...
q2 <- ggplot(data=simp.removed_na, aes(x=COG, y=Counts, fill=COG))+ geom_bar(stat="identity", width = 0.9, position=position_dodge()) + geom_text(aes(label = Counts), position=position_dodge(0.9),vjust=-0.2) + guides(fill = guide_legend(ncol = 1)) + xlab("COG Class")  +ggtitle("COG Results of Friedmanniomyces simplex \nDuplicated Genes") + scale_x_discrete(labels=xticks) + 
scale_fill_manual(values = getPalette(colourCount)) +  theme_bw()
q2
dev.off()
```
...
q3 <- ggplot(data=wer.removed_na, aes(x=COG, y=Counts, fill=COG))+ geom_bar(stat="identity", width = 0.9, position=position_dodge()) + geom_text(aes(label = Counts), position=position_dodge(0.9),vjust=-0.2) + guides(fill = guide_legend(ncol = 1)) + xlab("COG Class")  +ggtitle("COG Results of Hortaea werneckii \nDuplicated Genes") + scale_x_discrete(labels=xticks) + 
scale_fill_manual(values = getPalette(colourCount)) +  theme_bw()
q3
dev.off()
```
...
pdf("All_grid.pdf", width=20, height=6)
...
Legend_added <- plot_grid(prow, legend, rel_widths = c(3, 1.5))

Legend_added
ggsave("All_grid.png",Legend_added, width=20, height=6)
dev.off()
```
...
#cowplot - grid them together, remove legends, and have only one at the edge 
#https://github.com/wilkelab/cowplot/blob/master/vignettes/shared_legends.Rmd
pdf("Filtered_grid.pdf", width=20, height=6)
...
Filtered_Legend_added <- plot_grid(filtered_prow, legend, rel_widths = c(3, 1.5))

Filtered_Legend_added
ggsave("Filtered_grid.png",Filtered_Legend_added, width=20, height=6)
dev.off()
...
#cowplot - grid them together, remove legends, and have only one at the edge 
#https://github.com/wilkelab/cowplot/blob/master/vignettes/shared_legends.Rmd
pdf("Filtered_onlytwo_grid.pdf", width=11, height=8)
#Filtered_grid <-plot_grid(q1, q2, q3, labels = c('A', 'B', 'C'), label_size = 12)
filtered_prow <- plot_grid(
  q1 + theme(legend.position="none"),
  q2 + theme(legend.position="none"),
...
legend <- get_legend(
  # create some space to the left of the legend
  p1 + theme(legend.box.margin = margin(12, 0, 0, 12))
)
Filtered_Legend_added <- plot_grid(filtered_prow, legend, rel_widths = c(3, 2))

update<- Filtered_Legend_added + theme(aspect.ratio = .7)
update
ggsave("Filtered_onlytwo_grid.png",update, width=11, height=8)
dev.off()
